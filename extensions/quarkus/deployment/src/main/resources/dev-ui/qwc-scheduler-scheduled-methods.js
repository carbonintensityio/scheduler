...
export class QwcSchedulerScheduledMethods extends LitElement {
    ...
    _trigger(schedule) {
        ...
        if (schedule.fixedWindow) {
            trigger = html`${trigger} fixedWindow=<code>${schedule.fixedWindow}</code>`;
        }
        if (schedule.successive) {
            trigger = html`${trigger} successive=<code>${schedule.successive}</code>`;
        }
        if (schedule.cron) {
            trigger = schedule.cronConfig ? html`${trigger} <code>${schedule.cron}</code> configured as <code>${schedule.cronConfig}</code>` : html`${trigger} cron=<code>${schedule.cron}</code>`;
        }
        if (schedule.identity) {
